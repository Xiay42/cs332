...
char bbq_remove(BBQ *q)
{
    ...
    {
        condvar_wait(&q->cv_write, &q->lock);
    }
...
BBQ* bbq_init()
{
    ...
    condvar_init(&q->cv_write);
    condvar_init(&q->cv_read);
